-- MASTER FIX FOR RANKING AND STOPWATCH (V4)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'active_session_type') THEN
        ALTER TABLE profiles ADD COLUMN active_session_type TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'active_session_start') THEN
        ALTER TABLE profiles ADD COLUMN active_session_start TIMESTAMPTZ;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'active_session_initial_seconds') THEN
        ALTER TABLE profiles ADD COLUMN active_session_initial_seconds INTEGER DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'total_seconds') THEN
        ALTER TABLE profiles ADD COLUMN total_seconds INTEGER DEFAULT 0;
    END IF;
END $$;

-- 2. VIEW DE RANKING REAL-TIME
